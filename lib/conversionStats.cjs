...
} = require('./statsFilter.cjs');

function getSocialProof(db, ticker) {
  const sym = String(ticker || '')
    .trim()
    .toUpperCase();
  const logFilter = analysisLogIncludedInPublicStats();
  ...
  const usersTotal =
    db.prepare(`SELECT COUNT(*) AS c FROM users WHERE NOT ${excludedUsersWhere()}`).get()?.c ?? 0;
  const proPlusToday =
    db
      .prepare(
        `SELECT COUNT(*) AS c FROM analysis_logs
