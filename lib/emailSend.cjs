...
const { verificationCopy, verificationHtml, verificationText } = require('./emailTemplates.cjs');
const { getEmailTransport, isEmailConfigured } = require('./emailConfig.cjs');
...
async function sendVerificationEmail({ to, token, locale }) {
  const url = buildVerifyUrl(token);
  const copy = verificationCopy(locale);
  const text = verificationText({ url, locale });
  const html = verificationHtml({ url, locale });
  const result = await _sendMail({ to, subject: copy.subject, text, html });
  if (result.mode === 'console') {
    console.log(`[Wenap] 验证链接 (${to}): ${url}`);
    return { ...result, url };
  }
  return result;
}
  ...
  if (resendKey) {
    try {
      ...
      if (!resp.ok) {
        ...
        throw new Error(`Resend HTTP ${resp.status}${body ? `: ${body.slice(0, 200)}` : ''}`);
      }
