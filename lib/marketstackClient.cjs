...
/** Marketstack query plans — try exchange-qualified symbols before giving up. */
function marketstackFetchPlans(symbol) {
  const sym = sanitizeSymbol(symbol);
  if (!sym) return [];
  const plans = [];
  const seen = new Set();
  const add = (symbols, exchange = null) => {
    const key = `${symbols}|${exchange || ''}`;
    if (!symbols || seen.has(key)) return;
    seen.add(key);
    plans.push({ symbols, exchange });
  };

  add(sym);

  const tokyo = /^(\d{4})\.T$/i.exec(sym);
  if (tokyo) {
    const code = tokyo[1];
    plans.length = 0;
    seen.clear();
  ...
  const hk = /^(\d{4})\.HK$/i.exec(sym);
  if (hk) {
    add(hk[1], 'XHKG');
    add(`${hk[1]}.HK`);
  }
  const tw = /^(\d{4})\.TW$/i.exec(sym);
  if (tw) {
    add(tw[1], 'XTAI');
    add(`${tw[1]}.TW`);
  }
  const lon = /^([A-Z0-9]+)\.L$/i.exec(sym);
  if (lon) {
    add(lon[1], 'XLON');
    add(`${lon[1]}.L`);
  }
...
async function fetchEodRowsForPlan(plan, apiKey, limit = 260) {
  const params = { symbols: plan.symbols, limit, sort: 'DESC' };
  if (plan.exchange) params.exchange = plan.exchange;
  const eod = await marketstackJson('eod', params, apiKey);
  return Array.isArray(eod?.data) ? eod.data : [];
}
...
async function fetchMarketstackContextBundle(symbol, locale = 'zh-CN', apiKey = '') {
  ...
  if (!payload) {
    ...
    for (const plan of marketstackFetchPlans(sym)) {
      try {
        const rows = await fetchEodRowsForPlan(plan, apiKey, 260);
        const sumTry = summarizeFromEodRows(rows);
        if (sumTry && Number.isFinite(sumTry.close) && sumTry.close > 0) {
          eodRows = rows;
          resolvedPlan = plan;
          break;
        }
    ...
    payload = { eodRows, resolvedPlan };
    if (eodRows.length) await setCachedJson(cacheKey, payload, DEFAULT_TTL_SEC);
  }
...
async function fetchEodClosesOldestFirst(symbol, limit = 260, withVolume = false) {
  ...
  if (!payload) {
    ...
    for (const plan of marketstackFetchPlans(sym)) {
      try {
        const rows = await fetchEodRowsForPlan(plan, '', limit);
        const probe = summarizeFromEodRows(rows);
        if (probe && Number.isFinite(probe.close) && probe.close > 0) {
          eodRows = rows;
          break;
        }
      } catch {
...
/** Close on or before asOfDate (YYYY-MM-DD); uses same symbol plans as analyze. */
async function fetchClosePriceAsOf(symbol, asOfDate = new Date(), apiKey = '') {
  const target = asOfDate instanceof Date ? asOfDate : new Date(asOfDate);
  const targetYmd = target.toISOString().slice(0, 10);
  const key = String(apiKey || getMarketstackKey() || '').trim();
  if (!key) throw new Error('MARKETSTACK_NOT_CONFIGURED');

  for (const plan of marketstackFetchPlans(symbol)) {
    try {
      const rows = await fetchEodRowsForPlan(plan, key, 120);
  ...
  throw new Error('Marketstack 无法获取收盘价');
}
