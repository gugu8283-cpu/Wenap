...
const DIMENSIONS = {
  ...
  crypto: {
    'zh-CN': ['新闻情绪', '地缘政治', '链上活动', '宏观经济', '市场情绪', '监管与网络'],
    'zh-TW': ['新聞情緒', '地緣政治', '鏈上活動', '宏觀經濟', '市場情緒', '監管與網絡'],
    en: [
      'News sentiment',
      'Geopolitics',
      'On-chain activity',
    ...
    ja: ['ニュースセンチメント', '地政学', 'オンチェーン', 'マクロ経済', '市場センチメント', '規制・ネットワーク'],
    ko: ['뉴스 심리', '지정학', '온체인 활동', '거시경제', '시장 심리', '규제·네트워크'],
    de: [
      'Nachrichtenstimmung',
      'Geopolitik',
      'On-Chain-Aktivität',
      'Makroökonomie',
      'Marktstimmung',
      'Regulierung & Netzwerk',
    ],
  },
  forex: {
    'zh-CN': ['新闻情绪', '地缘政治', '利差', '宏观经济', '市场情绪', '央行政策分化'],
    'zh-TW': ['新聞情緒', '地緣政治', '利差', '宏觀經濟', '市場情緒', '央行政策分化'],
    en: [
      'News sentiment',
      'Geopolitics',
    ...
    ja: ['ニュースセンチメント', '地政学', '金利差', 'マクロ経済', '市場センチメント', '中央銀行政策'],
    ko: ['뉴스 심리', '지정학', '금리차', '거시경제', '시장 심리', '중앙은행 정책'],
    de: [
      'Nachrichtenstimmung',
      'Geopolitik',
      'Zinsdifferenz',
  ...
  commodities: {
    'zh-CN': ['新闻情绪', '地缘与供应', '供需平衡', '宏观经济', '市场情绪', '库存与季节性'],
    'zh-TW': ['新聞情緒', '地緣與供應', '供需平衡', '宏觀經濟', '市場情緒', '庫存與季節性'],
    en: [
      'News sentiment',
      'Geopolitics & supply',
    ...
    ja: ['ニュースセンチメント', '地政学・供給', '需給バランス', 'マクロ経済', '市場センチメント', '在庫・季節性'],
    ko: ['뉴스 심리', '지정학·공급', '수급 균형', '거시경제', '시장 심리', '재고·계절성'],
    de: [
      'Nachrichtenstimmung',
      'Geopolitik & Angebot',
      'Angebot/Nachfrage',
...
${n6} → 股息率、派息与 FFO/现金流覆盖、分红可持续性`;
  ...
  if (key === 'crypto') {
    if (loc === 'en') {
      return `【Six dimensions — crypto, fixed names】
${n1} → token/project news, exchange listings, social sentiment
${n2} → regulation, bans, sanctions affecting crypto markets
${n3} → on-chain activity, flows, staking, network usage (NOT equity industry trends)
${n4} → rates, USD, liquidity, macro risk appetite
${n5} → volatility, dominance, funding rates, positioning
${n6} → protocol upgrades, tokenomics, regulatory clarity`;
    }
    if (loc.startsWith('zh')) {
      return `【六维边界·加密货币 固定名称】
${n1} → 项目/代币新闻、上所、社区情绪
${n2} → 监管、禁令、制裁对加密市场影响
${n3} → 链上活跃度、资金流、质押（勿写股票行业周期）
${n4} → 利率、美元流动性、宏观风险偏好
${n5} → 波动率、BTC 主导率、资金费率
${n6} → 协议升级、代币经济、监管清晰度`;
    }
    return `【6 dims · crypto】${n.join(' | ')}. Dim3 = on-chain.`;
  }

  if (key === 'forex') {
    if (loc === 'en') {
      return `【Six dimensions — forex, fixed names】
${n1} → FX headlines, positioning, analyst views on the pair
${n2} → war, sanctions, capital controls affecting ${n[0] ? 'currencies' : 'FX'}
${n3} → rate differential, carry, forward points (NOT industry trends)
${n4} → GDP, inflation, employment, broad USD cycle
${n5} → technical levels, momentum, risk sentiment on the pair
${n6} → central bank guidance divergence, intervention risk`;
    }
    if (loc.startsWith('zh')) {
      return `【六维边界·外汇 固定名称】
${n1} → 货币对新闻、持仓、分析师观点
${n2} → 战争、制裁、资本管制
${n3} → 利差、套息、远期点数（勿写行业趋势）
${n4} → GDP、通胀、就业、美元周期
${n5} → 技术位、动能、风险偏好
${n6} → 央行指引分化、干预风险`;
    }
    return `【6 dims · forex】${n.join(' | ')}. Dim3 = rate differential.`;
  }

  if (key === 'commodities') {
    if (loc === 'en') {
      return `【Six dimensions — commodity spot, fixed names】
${n1} → commodity headlines, inventory reports, analyst tone
${n2} → supply disruption, OPEC/geopolitics, logistics
${n3} → supply/demand, inventories, curve structure (NOT equity industry)
${n4} → USD, rates, global growth impacting demand
${n5} → speculative positioning, volatility, term structure
${n6} → seasonal patterns, strategic reserves, producer discipline`;
    }
    if (loc.startsWith('zh')) {
      return `【六维边界·大宗商品现货 固定名称】
${n1} → 商品新闻、库存报告、市场情绪
${n2} → 供应中断、OPEC/地缘、物流
${n3} → 供需、库存、期限结构（勿写股票行业）
${n4} → 美元、利率、全球需求
${n5} → 投机持仓、波动、曲线
${n6} → 季节性、战略储备、生产商纪律`;
    }
    return `【6 dims · commodities】${n.join(' | ')}. Dim3 = supply/demand.`;
  }
...
评估实物/期货结构、展期与升贴水、费率、相对现货跟踪；上市商品 ETF 禁止 0 分。name 必须为「${dim}」。`;
  ...
  if (assetType === 'crypto' || assetType === 'forex' || assetType === 'commodities') {
    if (loc === 'en') {
      return `
【Dimension 6 · ${dim} — required】
Score 15–100 from verifiable facts for this asset class; never 0 unless symbol is invalid.
JSON dimensions[5].name MUST be exactly "${dim}".`;
    }
    if (loc.startsWith('zh')) {
      return `
【第6维·${dim} — 必须打分】
基于可核验事实打分 15–100；有效标的禁止轻易 0 分。dimensions[5].name 必须为「${dim}」。`;
    }
    return `【Dim6·${dim}】Score 15-100 required.`;
  }
...
- Do NOT output Chinese (Han) words or sentences when locale is English. The ONLY allowed Han characters are the single risk field values: 高, 中, or 低.
- JSON keys unchanged. signal: BUY | HOLD | SELL only. risk: exactly 高 or 中 or 低 (one character only; UI maps to High/Medium/Low).
...
- 中国語の文は出力しない。JSON キーは英語のまま。signal: BUY | HOLD | SELL。risk は 高 | 中 | 低 の1文字のみ（UI表示用。それ以外に中国語は禁止）。
- 次元名は下記スペックの日本語名を使用。`;
  }
  if (loc === 'ko') {
    return `
...
- 중국어 문장 금지. JSON 키는 영문 유지. signal: BUY | HOLD | SELL. risk는 高 | 中 | 低 한 글자만( UI용 ). 그 외 한자/중국어 금지.
...
- Keine chinesischen Sätze oder Wörter. JSON-Schlüssel unverändert. signal: BUY | HOLD | SELL. risk nur 高 | 中 | 低 (ein Zeichen, UI-Mapping).
...
/** Canonical risk tokens are 高|中|低 — map for display by locale. */
function riskDisplayLabel(risk, locale) {
  const r = String(risk || '').trim();
  const loc = normalizeLocale(locale);
  if (loc === 'en') {
    if (r === '高') return 'High';
    if (r === '中') return 'Medium';
    if (r === '低') return 'Low';
    return r || '—';
  }
  if (loc === 'ja') {
  ...
  if (loc === 'de') {
    if (r === '高') return 'Hoch';
    if (r === '中') return 'Mittel';
    if (r === '低') return 'Niedrig';
    return r || '—';
  }
  if (loc === 'ko') {
    if (r === '高') return '높음';
...
function credibilityDisplayLabel(cred, locale) {
  const c = String(cred || '').trim().toLowerCase();
  const loc = normalizeLocale(locale);
  if (loc === 'en') {
    if (c === '高' || c === 'high') return 'High';
    if (c === '低' || c === 'low') return 'Low';
    if (c === '中' || c === 'mid' || c === 'medium') return 'Med';
    return cred || 'Med';
  }
  if (loc.startsWith('zh')) {
  ...
  if (c === 'high' || c === '高') return loc === 'de' ? 'Hoch' : loc === 'ko' ? '높음' : '高';
  if (c === 'low' || c === '低') return loc === 'de' ? 'Niedrig' : loc === 'ko' ? '낮음' : '低';
  if (c === 'mid' || c === 'medium' || c === '中') return loc === 'de' ? 'Mittel' : loc === 'ko' ? '중' : '中';
  return String(cred || '—');
}

/** Replace stray 高|中|低 (and common LLM phrases) in non-CJK UI locales. */
function localizeRiskCredTokensInText(text, locale) {
  const loc = normalizeLocale(locale);
  if (loc === 'zh-CN' || loc === 'zh-TW' || loc === 'ja') return String(text || '');
  let t = String(text || '');
  if (!t || !/[高中低]/.test(t)) return t;

  const rHigh = riskDisplayLabel('高', loc);
  const rMid = riskDisplayLabel('中', loc);
  const rLow = riskDisplayLabel('低', loc);
  const cHigh = credibilityDisplayLabel('高', loc);
  const cMid = credibilityDisplayLabel('中', loc);
  const cLow = credibilityDisplayLabel('低', loc);

  t = t.replace(/Risk score\s*高/gi, `${rHigh} risk score`);
  t = t.replace(/Risk score\s*中/gi, `${rMid} risk score`);
  t = t.replace(/Risk score\s*低/gi, `${rLow} risk score`);
  t = t.replace(/Risk level\s*[:：]\s*高/gi, `Risk level: ${rHigh}`);
  t = t.replace(/Risk level\s*[:：]\s*中/gi, `Risk level: ${rMid}`);
  t = t.replace(/Risk level\s*[:：]\s*低/gi, `Risk level: ${rLow}`);
  t = t.replace(/\*\*Risk level:\s*高\*\*/gi, `**Risk level: ${rHigh}**`);
  t = t.replace(/\*\*Risk level:\s*中\*\*/gi, `**Risk level: ${rMid}**`);
  t = t.replace(/\*\*Risk level:\s*低\*\*/gi, `**Risk level: ${rLow}**`);
  t = t.replace(/credibility\s*[:：]?\s*高/gi, `credibility: ${cHigh}`);
  t = t.replace(/credibility\s*[:：]?\s*中/gi, `credibility: ${cMid}`);
  t = t.replace(/credibility\s*[:：]?\s*低/gi, `credibility: ${cLow}`);

  t = t.replace(/(?<=[\s(,:/]|^)(高|中|低)(?=[\s).,;]|$)/g, (ch) => riskDisplayLabel(ch, loc));
