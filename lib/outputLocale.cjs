...
function dimensionJsonSpec(assetType, locale) {
  ...
  const noteCap =
    loc === 'en'
      ? 'note≤48 words, 1–2 hard facts + [cite]; no repeat of summary/other dims; empty note if score=0'
      : loc.startsWith('zh')
...
function dimensionBoundaryPromptBlock(locale) {
  const loc = normalizeLocale(locale);
  if (loc === 'en') {
    return `【Six dimensions — no overlap】
News sentiment → recent media, analyst ratings, buzz
Geopolitics → country-level conflict, war risk, regional supply chains (not corporate regulation)
Industry trends → growth, tech cycles, competition
Macroeconomics → rates, inflation, GDP, jobs, broad market
Market sentiment → investor mood, valuation, flows, momentum
Policy & regulation → government/regulator actions on the company or sector (export controls, AI laws, antitrust, sector rules, tax, IP); not geopolitics`;
  }
  if (loc === 'ja') {
    return `【6次元の境界】各次元は定義内のみ。ニュース／地政学／業界／マクロ／市場センチ／政策・規制を混同しない。`;
  }
...
政策法规 → 政府/监管机构对该公司或行业的监管、立法、执法；勿与地缘政治混淆`;
}
