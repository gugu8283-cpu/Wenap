 ...
 */
const { normalizeLocale, localizeRiskCredTokensInText } = require('./outputLocale.cjs');
...
function localizeReportCitations(data, locale) {
  ...
  for (const k of REPORT_TEXT_KEYS) {
    if (typeof data[k] === 'string') data[k] = locText(data[k], locale);
  }
  ...
  if (data.actionLineObj && typeof data.actionLineObj === 'object') {
    for (const k of ['suggestion', 'stopLoss', 'catalyst']) {
      if (typeof data.actionLineObj[k] === 'string') {
        data.actionLineObj[k] = locText(data.actionLineObj[k], locale);
      }
    }
  }
  ...
  if (data.coreConclusion && typeof data.coreConclusion === 'object') {
    for (const k of ['headline', 'ifBull', 'ifBear', 'action', 'bullCase', 'bearCase']) {
      if (typeof data.coreConclusion[k] === 'string') {
        data.coreConclusion[k] = locText(data.coreConclusion[k], locale);
      }
    }
  }
  ...
  for (const d of dims) {
    if (d && typeof d.note === 'string') d.note = locText(d.note, locale);
  }
  ...
  if (data.scenarios && typeof data.scenarios === 'object') {
    for (const k of ['bull', 'base', 'bear']) {
      ...
      if (typeof z.trigger === 'string') z.trigger = locText(z.trigger, locale);
      if (typeof z.range === 'string') z.range = locText(z.range, locale);
    }
  }
  ...
  for (const c of chain) {
    ...
    if (typeof c.reason === 'string') c.reason = locText(c.reason, locale);
    if (typeof c.relation === 'string') c.relation = locText(c.relation, locale);
    if (typeof c.analysis === 'string') c.analysis = locText(c.analysis, locale);
  }

  if (data.secondPassCritique && typeof data.secondPassCritique === 'object') {
    const sp = data.secondPassCritique;
    if (typeof sp.blindSpot === 'string') sp.blindSpot = locText(sp.blindSpot, locale);
    if (Array.isArray(sp.weaknesses)) {
      sp.weaknesses = sp.weaknesses.map((w) => locText(w, locale));
    }
  }

  if (Array.isArray(data.criticAngles)) {
    data.criticAngles = data.criticAngles.map((w) => locText(w, locale));
  }
  ...
  if (data.bullBearDebate && typeof data.bullBearDebate === 'object') {
    const normSide = (arr) =>
      (Array.isArray(arr) ? arr : []).map((x) => {
        ...
        const reason = typeof x.reason === 'string' ? locText(x.reason, locale) : x.reason;
        const text = typeof x.text === 'string' ? locText(x.text, locale) : x.text;
        return { ...x, ...(reason != null ? { reason } : {}), ...(text != null ? { text } : {}) };
      });
    data.bullBearDebate = {
      bull: normSide(data.bullBearDebate.bull),
      bear: normSide(data.bullBearDebate.bear),
    };
  }
  ...
  data.sources = arr.map((s) => {
    ...
    const text =
      typeof s.text === 'string' ? locText(s.text, locale) : s.text;
