...
function dataFreshnessPromptBlock(loc) {
...
function yearFromUrlPath(url) {
  const m = String(url || '').match(/\/(20\d{2})\/(0[1-9]|1[0-2])\//);
  return m ? Number.parseInt(m[1], 10) : null;
}

function tagUrlDateMismatch(text, loc) {
  const t = String(text || '').trim();
  if (!t || /url\/date mismatch|链接与日期/i.test(t)) return t;
  return String(loc || '').startsWith('zh')
    ? `${t} ⚠️ 链接与日期可能不一致`
    : `${t} ⚠️ URL/date mismatch`;
}
...
function enforceReportAccuracy(data, ctx = {}) {
  ...
  const zh = loc === 'zh-CN' || loc === 'zh-TW';
  const warnings = [];
  const snap = ctx.marketSnapshot && typeof ctx.marketSnapshot === 'object' ? ctx.marketSnapshot : null;

  const gq = ctx.globalQuote && typeof ctx.globalQuote === 'object' ? ctx.globalQuote : null;
  const tradingDay =
    snap?.tradingDay ||
    (gq ? String(gq['07. latest trading day'] || '').trim() : '');
  ...
  if (snap?.priceStaleNotice && !warnings.includes(snap.priceStaleNotice)) {
    warnings.push(snap.priceStaleNotice);
  }
  if (snap?.priceStaleOver7Days && tradingDay) {
    const extra = zh
      ? `⚠️ 锁定现价口径截至 ${tradingDay}，超过 7 天须在正文中标注日期`
      : `⚠️ Locked spot price as of ${tradingDay} (>7d); cite the date in narrative`;
    if (!warnings.some((w) => String(w).includes(tradingDay))) warnings.push(extra);
  }
  if (Array.isArray(snap?.fieldFreshnessWarnings)) {
    for (const w of snap.fieldFreshnessWarnings) {
      if (w && !warnings.includes(w)) warnings.push(w);
    }
  }
  if (Number.isFinite(snap?.currentPrice) && snap.currentPrice > 0) {
    data.avCurrentPrice = snap.currentPrice;
    if (snap.priceAsOfDisplay) data.priceAsOfDisplay = snap.priceAsOfDisplay;
  }
  ...
  for (const s of sources) {
    ...
    let d = parseYmd(s.time);
    const urlYear = yearFromUrlPath(s.url);
    const timeYear = d ? d.getUTCFullYear() : null;
    if (urlYear && timeYear && Math.abs(urlYear - timeYear) > 1) {
      urlMismatchCount += 1;
      s._urlDateMismatch = true;
      s.credibility = downgradeCredibility(s.credibility);
      s.text = tagUrlDateMismatch(s.text, loc);
      s.time = `${urlYear}-01-01`;
      d = parseYmd(s.time);
    }
    if (!d) {
  ...
  if (urlMismatchCount > 0) {
    warnings.push(
      zh
        ? `${urlMismatchCount} 条来源链接路径年份与标注日期不一致，已降级并标注`
        : `${urlMismatchCount} source(s) have URL path vs date mismatch; downgraded`,
    );
  }
  ...
  freshnessScore -= Math.min(20, urlMismatchCount * 10);
